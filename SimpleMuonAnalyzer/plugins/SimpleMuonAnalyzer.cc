...
SimpleMuonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if ((recoMuons.size() < 10) and (recoMuons.size() > 1)) {
	...
	std::cout << "muon eta at vertex: " << recoMuon.eta() << " " << "phi: " << recoMuon.phi() << std::endl;
	std::cout << "stepping helix eta at St2: " << prop_point.eta() << " " << "phi: " <<prop_point.phi() << std::endl;
	//std::cout << "-----------------" << std::endl;
      }
	...
	if (r.detector()==2 and r.station()==2) {
	  ...
	  std::cout << "lp -> gp -> eta at St2: " << gp.eta() << ", phi: " << gp.phi() << std::endl;
	  break;
	}
      }
